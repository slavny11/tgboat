...
//        await messageHandler(app: app, connection: connection)
        await sendWeekdayGarbage(app: app, connection: connection)
...
//        await buttonsActionHandler(app: app, connection: connection)
    ...
    private static func sendWeekdayGarbage(app: Vapor.Application, connection: TGConnectionPrtcl) async {
        await connection.dispatcher.add(TGMessageHandler(filters: .all) { update, bot in
            guard let message = update.message else { return }

            let weekdays = Weekdays()
            let todayMessage = weekdays.getTheMessage()

            let params: TGSendMessageParams = .init(chatId: .chat(message.chat.id), text: todayMessage)
    ...
    private static func defaultBaseHandler(app: Vapor.Application, connection: TGConnectionPrtcl) async {
        await connection.dispatcher.add(TGBaseHandler({ update, bot in
            guard let message = update.message else { return }
            let params: TGSendMessageParams = .init(chatId: .chat(message.chat.id), text: "oops")
            try await bot.sendMessage(params: params)
        }))
    }
